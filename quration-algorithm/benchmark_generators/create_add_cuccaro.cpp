...
struct AddCuccaroParams {
    std::size_t size;
};

void from_json(const nlohmann::json& j, AddCuccaroParams& p) {
    p.size = j.at("size").get<std::size_t>();
}

AddCuccaroParams LoadAddCuccaroJson(const std::string& path) {
    ...
    nlohmann::json j;
    ifs >> j;
    return j.get<AddCuccaroParams>();
};

int main(std::int32_t argc, const char* const* const argv) {
    namespace po = boost::program_options;
    po::options_description desc(
            "Create AddCuccaro circuit from JSON file\n\n"
            "Input JSON fields:\n"
    ...
    const auto params = LoadAddCuccaroJson(input_file);

    qret::ir::IRContext context;
    auto* module = qret::ir::Module::Create("AddCuccaroModule", context);
    auto builder = qret::frontend::CircuitBuilder(module);
    auto gen = qret::frontend::gate::AddCuccaroGen(&builder, params.size);
